import streamlit as st
import pandas as pd
import plotly.express as px
import plotly.graph_objects as go
from datetime import date
import time

from src.database import engine, Base, get_db
from src.auth import create_user, get_user_by_username, verify_password
import src.models

from src.services.transaction_service import TransactionService
from src.services.budget_service import BudgetService
from src.services.ml_service import MLService
from src.services.export_service import ExportService

# Cria tabelas se não existirem
Base.metadata.create_all(bind=engine)

# ======================================================
# CONFIGURAÇÃO DA PÁGINA
# ======================================================
st.set_page_config(
    page_title="Finance Pro - Gestão de Gastos",
    page_icon="💸",
    layout="wide",
    initial_sidebar_state="expanded"
)

def load_css():
    try:
        with open("src/ui/styles.css") as f:
            st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)
    except FileNotFoundError:
        pass

load_css()

# ======================================================
# TELA DE LOGIN
# ======================================================
def login_screen():
    col1, col2, col3 = st.columns([1, 1, 1])
    with col2:
        st.markdown("<h1 style='text-align: center; margin-bottom: 2rem;'>💳 Finance Pro</h1>", unsafe_allow_html=True)
        
        tab_login, tab_reg = st.tabs(["Acessar", "Registrar"])
        
        with tab_login:
            with st.form("login_form"):
                username = st.text_input("Usuário")
                password = st.text_input("Senha", type="password")
                if st.form_submit_button("Entrar", use_container_width=True, type="primary"):
                    with get_db() as db:
                        user = get_user_by_username(db, username)
                        if user and verify_password(password, user.password_hash):
                            st.session_state.logged_in = True
                            st.session_state.user_id = user.id
                            st.session_state.username = user.username
                            st.rerun()
                        else:
                            st.error("Credenciais inválidas.")
        
        with tab_reg:
            with st.form("register_form"):
                new_user = st.text_input("Usuário")
                new_pass = st.text_input("Senha", type="password")
                if st.form_submit_button("Criar Conta", use_container_width=True):
                    with get_db() as db:
                        if create_user(db, new_user, "email@fake.com", new_pass):
                            st.success("Sucesso! Faça login.")
                        else:
                            st.error("Usuário já existe.")

# ======================================================
# DASHBOARD PRINCIPAL
# ======================================================
def dashboard_screen():
    # --- Sidebar ---
    with st.sidebar:
        st.markdown(f"### 👤 {st.session_state.username}")
        st.caption("Perfil Pessoal")
        st.markdown("---")
        if st.button("🚪 Sair", use_container_width=True):
            st.session_state.clear()
            st.rerun()

    user_id = st.session_state.user_id
    
    # --- Header com Saudação ---
    hora = pd.Timestamp.now().hour
    saudacao = "Bom dia" if 5 <= hora < 12 else "Boa tarde" if 12 <= hora < 18 else "Boa noite"
    st.title(f"{saudacao}, {st.session_state.username}!")
    st.markdown("Visão geral da sua saúde financeira.")

    with get_db() as db:
        # Instancia Serviços
        srv_trans = TransactionService(db)
        srv_budget = BudgetService(db)
        srv_ml = MLService(db) # Passamos DB para acessar histórico no ML
        srv_export = ExportService()

        # 1. Carregar Dados Totais
        df = srv_trans.df_usuario(user_id)

        if df.empty:
            st.info("👋 Bem-vindo! Vá em **Movimentações** para começar seus registros.")
            return

        # 2. Filtros de Data (Mês/Ano)
        df["date"] = pd.to_datetime(df["date"])
        
        with st.container():
            col_date1, col_date2, spacer = st.columns([1, 1, 4])
            with col_date1:
                anos = sorted(df["date"].dt.year.unique(), reverse=True)
                ano_sel = st.selectbox("Ano", anos, key="year_sel")
            with col_date2:
                # Mostra apenas meses disponíveis naquele ano
                meses_disp = sorted(df[df["date"].dt.year == ano_sel]["date"].dt.month.unique(), reverse=True)
                mes_map = {1:"Jan", 2:"Fev", 3:"Mar", 4:"Abr", 5:"Mai", 6:"Jun", 7:"Jul", 8:"Ago", 9:"Set", 10:"Out", 11:"Nov", 12:"Dez"}
                
                # Se não tiver meses (ex: deletou tudo), evita erro
                if not meses_disp: meses_disp = [date.today().month]

                mes_sel = st.selectbox("Mês", meses_disp, format_func=lambda x: mes_map.get(x, str(x)), key="month_sel")

        # Filtra o DataFrame Principal
        df_filtrado = df[(df["date"].dt.year == ano_sel) & (df["date"].dt.month == mes_sel)]

        # 3. KPIs Cards
        receita = df_filtrado[df_filtrado["type"] == "renda"]["amount"].sum()
        gasto = df_filtrado[df_filtrado["type"] == "gasto"]["amount"].sum()
        saldo = receita - gasto

        col1, col2, col3, col4 = st.columns(4)
        col1.metric("Receitas", f"R$ {receita:,.2f}", delta="Mensal")
        col2.metric("Despesas", f"R$ {gasto:,.2f}", delta="-Saídas", delta_color="inverse")
        col3.metric("Saldo", f"R$ {saldo:,.2f}", delta="Residuo", delta_color="off")
        
        # KPI 4: ML Projeção Inteligente
        hoje = date.today()
        is_current_month = (hoje.year == ano_sel) and (hoje.month == mes_sel)
        
        if is_current_month:
            resultado_ml = srv_ml.calcular_projecao_inteligente(user_id, df_filtrado)
            
            # Tratamento de erro/compatibilidade
            if isinstance(resultado_ml, dict):
                projecao_val = resultado_ml["valor"]
                metodo_msg = resultado_ml["metodo"]
            else:
                projecao_val = resultado_ml
                metodo_msg = "Estimativa"

            col4.metric(
                "Projeção Fim do Mês", 
                f"R$ {projecao_val:,.2f}", 
                delta=f"Base: {metodo_msg}", 
                delta_color="off" 
            )
        else:
            col4.metric("Status", "Mês Fechado", delta="Finalizado", delta_color="off")

        st.markdown("---")

        # 4. Gráficos Principais (COM A LÓGICA NOVA DE BARRAS)
        c_chart1, c_chart2 = st.columns([1, 2])

        with c_chart1:
            st.subheader("Gastos por Categoria")
            df_gasto = df_filtrado[df_filtrado["type"] == "gasto"]
            
            if not df_gasto.empty:
                # --- LÓGICA INTELIGENTE DE GRÁFICO ---
                # Se tiver mais de 5 categorias, usa Barras Horizontais para não poluir
                if len(df_gasto) > 5:
                    # Ordena: Menor para Maior (o Plotly plota de baixo pra cima, então o maior fica no topo)
                    df_gasto = df_gasto.sort_values(by="amount", ascending=True)
                    
                    fig = px.bar(
                        df_gasto, 
                        x="amount", 
                        y="category", 
                        orientation='h',
                        text="amount",
                        color="category",
                        color_discrete_sequence=px.colors.qualitative.Pastel
                    )
                    
                    # Formatação das Barras
                    fig.update_traces(
                        texttemplate='R$ %{x:.2s}', # Formato resumido (ex: 1.2k) ou use %{x:,.2f}
                        textposition='auto'
                    )
                    
                    fig.update_layout(
                        showlegend=True,
                        margin=dict(t=10, b=10, l=10, r=10),
                        height=350,
                        xaxis=dict(showgrid=False, showticklabels=False, color="white"), # Esconde eixo X
                        yaxis=dict(showgrid=False, color="white"),
                        paper_bgcolor="rgba(0,0,0,0)",
                        plot_bgcolor="rgba(0,0,0,0)",
                        font=dict(color="white")
                    )
                    st.plotly_chart(fig, use_container_width=True)

                else:
                    # Se tiver 5 ou menos, mantém o Donut Chart (Bonito e minimalista)
                    fig_pizza = go.Figure(data=[go.Pie(
                        labels=df_gasto['category'], 
                        values=df_gasto['amount'], 
                        hole=.65,
                        textinfo='percent', 
                        textposition='outside',
                        marker=dict(colors=px.colors.qualitative.Pastel)
                    )])
                    
                    fig_pizza.update_layout(
                        showlegend=True, 
                        legend=dict(orientation="h", y=-0.1),
                        annotations=[dict(
                            text=f'R$ {gasto:,.0f}', 
                            x=0.5, y=0.5, 
                            font_size=30, 
                            showarrow=False, 
                            font_color="white"
                        )],
                        margin=dict(t=10, b=10, l=10, r=10),
                        paper_bgcolor="rgba(0,0,0,0)",
                        plot_bgcolor="rgba(0,0,0,0)",
                        font=dict(color="white"),
                        height=350
                    )
                    st.plotly_chart(fig_pizza, use_container_width=True)

            else:
                st.info("Sem gastos registrados.")

        with c_chart2:
            st.subheader("Balanço Diário")
            df_diario = df_filtrado.groupby([df_filtrado["date"].dt.day, "type"])["amount"].sum().reset_index()
            df_diario.columns = ["Dia", "Tipo", "Valor"]
            
            if not df_diario.empty:
                fig_bar = px.bar(
                    df_diario, 
                    x="Dia", 
                    y="Valor", 
                    color="Tipo", 
                    barmode="group",
                    color_discrete_map={"renda": "#2ecc71", "gasto": "#e74c3c"}
                )
                fig_bar.update_layout(
                    margin=dict(t=10, b=10, l=10, r=10),
                    height=350,
                    xaxis=dict(showgrid=False, color="white", tickmode='linear'),
                    yaxis=dict(showgrid=True, gridcolor="#333", color="white"),
                    legend=dict(orientation="h", y=1.1, title="", font=dict(color="white")),
                    paper_bgcolor="rgba(0,0,0,0)",
                    plot_bgcolor="rgba(0,0,0,0)"
                )
                st.plotly_chart(fig_bar, use_container_width=True)
            else:
                st.info("Sem movimentações.")

        # 5. Radar Financeiro (IA)
        st.subheader("🕵️‍♂️ Radar Financeiro")
        anomalias = srv_ml.detectar_anomalias(df_filtrado)
        
        if not anomalias.empty:
            st.caption("O sistema detectou transações fora do seu padrão habitual.")
            for _, row in anomalias.iterrows():
                with st.expander(f"🔴 {row['category']} - R$ {row['amount']:,.2f}", expanded=True):
                    st.write(f"**Data:** {row['date'].strftime('%d/%m/%Y')}")
                    st.write(f"**Descrição:** {row['description'] or 'Sem descrição'}")
                    st.caption("Este valor foge do seu padrão habitual para esta categoria.")
        else:
            if df_filtrado.empty:
                st.caption("Sem dados para análise.")
            else:
                st.success("Nenhuma anomalia detectada. Seus gastos seguem o padrão habitual.", icon="🛡️")

        st.markdown("---")

        # 6. Últimas Transações e Orçamentos
        col_list, col_budget = st.columns([2, 1])
        
        with col_list:
            st.subheader("⏱️ Últimas Movimentações")
            if not df_filtrado.empty:
                last_trans = df_filtrado.sort_values("date", ascending=False).head(5)
                for _, row in last_trans.iterrows():
                    with st.container():
                        c1, c2, c3, c4 = st.columns([0.5, 2, 2, 1.5])
                        icon = "🔻" if row["type"] == "gasto" else "💚"
                        cor_valor = "red" if row["type"] == "gasto" else "green"
                        
                        c1.write(f"### {icon}")
                        c2.write(f"**{row['category']}**")
                        c2.caption(row["date"].strftime("%d/%m"))
                        c3.write(f"{row['description'] or '-'}")
                        c4.markdown(f":{cor_valor}[**R$ {row['amount']:,.2f}**]")
                        st.divider()
            else:
                st.caption("Nada recente.")

        with col_budget:
            st.subheader("🎯 Orçamentos")
            alertas = srv_budget.alertas(user_id, df_filtrado)
            if alertas:
                for a in alertas: st.error(a)
            else:
                st.info("Tudo dentro do planejado.")

def main():
    if "logged_in" not in st.session_state:
        st.session_state.logged_in = False
        st.session_state.user_id = None
        st.session_state.username = None

    if st.session_state.logged_in:
        dashboard_screen()
    else:
        login_screen()

if __name__ == "__main__":
    main()